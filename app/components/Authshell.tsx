      ...
      >
        ...
        <div className="relative hidden flex-col justify-between overflow-hidden bg-linear-to-br from-marica-cream via-marica-amber/25 to-marica-rose/40 p-10 lg:flex">
          ...
          <div className="relative z-10">
            <span className="font-display text-lg font-semibold text-marica-amber-text">
              Marica
            </span>
            ...
            <p className="mt-3 max-w-xs font-body text-[15px] leading-relaxed text-marica-ink-soft">
              Satu akun untuk semua progress calistung si kecil — dari kelas
              online sampai Edu-Kit bulanan.
            </p>
            ...
            <ul className="mt-8 flex flex-col gap-3">
              {highlights.map(({ icon: Icon, text }) => (
                <li
                  key={text}
                  className="flex items-start gap-2.5 font-body text-sm text-marica-ink-soft"
                >
          ...
          <AnimatePresence mode="wait">
            {resolvingOAuth ? (
              ...
              >
                ...
                <p className="font-body text-[15px] font-medium text-marica-ink">
                  Berhasil masuk!
                </p>
                <p className="font-body text-sm text-marica-ink-soft">
                  Mengarahkan kamu ke halaman yang tepat...
                </p>
              </motion.div>
            ) : (
              <motion.div
              ...
              >
                ...
                >
                  ...
                  <div>
                    <div className="mb-1.5 flex items-center justify-between">
                      <label
                        htmlFor="password"
                        className="block font-body text-sm font-medium text-marica-ink"
                      >
                        Password
                      </label>
                      {isLogin && (
                    ...
                    <label className="flex items-start gap-2.5 font-body text-[13px] text-marica-ink-soft">
                      ...
                      <span>
                        ...
                        <Link
                          ...
                          className="font-medium text-marica-amber-text underline underline-offset-2"
                        >
                          Syarat &amp; Ketentuan
                        </Link>{" "}
                        serta{" "}
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const params = new URLSearchParams(window.location.search);
                    const existingCallbackUrl = getSafeRelativePath(
                      params.get("callbackUrl"),
                    );
                    const target = `${isLogin ? "/login" : "/daftar"}?oauth=1${
                      existingCallbackUrl
                        ? `&callbackUrl=${encodeURIComponent(existingCallbackUrl)}`
                        : ""
                    }`;
